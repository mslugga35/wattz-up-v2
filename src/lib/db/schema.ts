...
export const stations = pgTable('wattz_stations', {
  ...
  index('idx_wattz_stations_geohash6').on(table.geohash6),
  index('idx_wattz_stations_network').on(table.network),
  index('idx_wattz_stations_source').on(table.source),
]);

// ============ OBSERVATIONS TABLE ============

export const observations = pgTable('wattz_observations', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  index('idx_wattz_observations_station').on(table.stationId),
  index('idx_wattz_observations_observed').on(table.observedAt),
  index('idx_wattz_observations_type').on(table.observationType),
]);

// ============ USERS TABLE ============

export const users = pgTable('wattz_users', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  uniqueIndex('idx_wattz_users_device').on(table.deviceId),
]);

// ============ ALERTS TABLE ============

export const alerts = pgTable('wattz_alerts', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  index('idx_wattz_alerts_user').on(table.userId),
  index('idx_wattz_alerts_station').on(table.stationId),
  index('idx_wattz_alerts_status').on(table.status),
]);

// ============ SESSION STATS (for estimation) ============

export const sessionStatsHourly = pgTable('wattz_session_stats_hourly', {
  id: uuid('id').primaryKey().defaultRandom(),
  stationId: uuid('station_id').notNull().references(() => stations.id),
  ...
  index('idx_wattz_session_stats_station').on(table.stationId),
  index('idx_wattz_session_stats_time').on(table.dayOfWeek, table.hourOfDay),
]);

// ============ INGEST JOBS (for tracking data imports) ============

export const ingestJobs = pgTable('wattz_ingest_jobs', {
  id: uuid('id').primaryKey().defaultRandom(),
